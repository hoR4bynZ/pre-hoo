...
void __createThread (struct pcb* pthread, __threadFunc function, void* funcArg) {
    // enum _intrStatus oldStatus = __intrDisable();                   //关中断
    // __consolePrintStr("\n(create thd)original stack is: ");
    ...
    // __consolePrintStr(" = ");
    //线程0特权级栈 减去 中断栈
    pthread->kStack = pthread->kStack - sizeof(struct intrStack);
    // __consolePrintInt(pthread->kStack);
    ...
    // __consolePrintStr(" = ");
    //再减去 线程栈
    pthread->kStack = pthread->kStack - sizeof(struct thdStack);
    // __consolePrintInt(pthread->kStack);
    // __consolePrintStr("\n\n");

    // __intrSetStatus(oldStatus);                                     //恢复原来状态
...
void __schedule(){
    ...
    __processActivate(next);                                        //根据是进程还是线程来更新页表地址以及0级栈
