...
static void __initMemPool (uint32 memSize) {
    ...
    __lockInit(&pkernel.lock);
    __lockInit(&puser.lock);
...
static void* __valloc (enum poolFlag pf, uint32 pCnt) {
    ...
    if (pf == PF_KERNEL) {                              //内核虚拟池
        ...
        idxBtmpBase = __btmpScan(&cur->usrvAddr.vaddrBtmp, pCnt);
        if(idxBtmpBase == -1){
            return NULL;
        }
        while(cnt < pCnt){
            __btmpSet(&cur->usrvAddr.vaddrBtmp, idxBtmpBase + cnt++, 1);
        }
        vaddrBase = cur->usrvAddr.vaddrBase + idxBtmpBase * PG_SIZE;
        ASSERT((uint32)vaddrBase < (0xc0000000 - PG_SIZE));
    }
...
void* __upageAlloc(uint32 pCnt){
    __lockAcquire(&puser.lock);
    void* vaddr = __pallocN(PF_USER, pCnt);
    __memset(vaddr, 0, pCnt * PG_SIZE);
    __lockRelease(&puser.lock);
    return vaddr;
}
...
void* __apageAlloc(enum poolFlag pf, uint32 vaddr){
    //申请一页自定义映射的内存页
    struct _phyAddr* pool = pf & PF_KERNEL ? &pkernel : &puser;
    __lockAcquire(&pool->lock);

    //修改虚拟地址池位图
    struct pcb* cur = __thdAddr();
    int32 idxBit = -1;
    if(cur->ptvAddr != NULL && pf == PF_USER){                      //页表地址非空，此时是用户进程
        idxBit = (vaddr - cur->usrvAddr.vaddrBase) / PG_SIZE;       //页数偏移/页单位 = 位偏移
        ASSERT(idxBit > 0);
        __btmpSet(&cur->usrvAddr.vaddrBtmp, idxBit, 1);             //找到位图的相应位置1
    }else if(cur->ptvAddr == NULL && pf == PF_KERNEL){
        idxBit = (vaddr - vkernel.vaddrBase) / PG_SIZE;
        ASSERT(idxBit > 0);
        __btmpSet(&vkernel.vaddrBtmp, idxBit, 1);
    }else{
        PANIC("ERROR: __apageAlloc not allow kernel/user to alloc userspace in memory.c\n");
    }

    //分配物理页
    void* pPage = __palloc(pool);
    if(!pPage){
        return NULL;
    }

    //虚实映射
    __pageTableAdd((void*)vaddr, pPage);

    __lockRelease(&pool->lock);
    return (void*)vaddr;
}
...
uint32 __vir2phy(uint32 vaddr){
    uint32* pte = __ptrPte(vaddr);          //获取虚拟地址所映射的物理页上级的页表项
    //pte是虚拟地址第二级的页表项所指向的页表地址，加上虚拟地址低12位即物理页地址
